...
try:
    ...
    with st.container():
        ...
        with filter_col2:
            secilen_marka = st.selectbox("🏷️ Marka", ["Tümü"] + list(df[marka_col].dropna().unique()), key="secilen_marka")
        with filter_col3:
            secilen_grup = st.selectbox("📂 Ürün Grubu", ["Tümü"] + list(df[grup_col].dropna().unique()), key="secilen_grup")
        with filter_col4:
    ...
    st.dataframe(
        ...
        hide_index=True,  # İstenilen sıra numarası gizleme
        height=600
    )
