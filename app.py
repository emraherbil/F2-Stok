...
try:
    ...
    c_satis_acik = next(
        (c for c in df.columns if "açık" in str(c).lower() or "acik" in str(c).lower()),
        None,
    )
    ...
    st.markdown(
        f"<hr style='margin-top:10px; margin-bottom:20px; border-color:{'#333333' if is_dark else '#e0e0e0'};'>",
        unsafe_allow_html=True,
    )

    # ==========================================
    # 4. FRAGMENT ALANI (FİLTRE VE TABLO)
    # ==========================================
    @st.fragment
    def stok_paneli_icerik(data_frame):
        ...
        marka_ops = ["Tümü"] + sorted([
            str(x) for x in df_for_marka[c_marka].dropna().unique() if str(x).lower() != "nan"
        ])
        ...
        grup_ops = ["Tümü"] + sorted([
            str(x) for x in df_for_grup[c_grup].dropna().unique() if str(x).lower() != "nan"
        ])
        ...
        with col1:
            st.markdown(
                f"<div style='color: {label_color}; font-size: 14px; margin-bottom: 6px; font-weight: normal;'>📝 Ürün Ara</div>",
                unsafe_allow_html=True,
            )
        ...
        with col2:
            st.markdown(
                f"<div style='color: {label_color}; font-size: 14px; margin-bottom: 6px; font-weight: normal;'>🏷️ Marka</div>",
                unsafe_allow_html=True,
            )
            v_marka = st.selectbox("Marka", marka_ops, label_visibility="collapsed", key="q_marka")

        with col3:
            st.markdown(
                f"<div style='color: {label_color}; font-size: 14px; margin-bottom: 6px; font-weight: normal;'>📂 Ürün Grubu</div>",
                unsafe_allow_html=True,
            )
            v_grup = st.selectbox("Ürün Grubu", grup_ops, label_visibility="collapsed", key="q_grup")
        ...
        with k1:
            st.markdown(
                kpi_card("📋 Toplam Çeşit:", f"{t_prod:,}".replace(",", ".") + " Adet", "#1E88E5"),
                unsafe_allow_html=True,
            )
        with k2:
            st.markdown(
                kpi_card("📦 Toplam Stok:", f"{t_stok:,}".replace(",", ".") + " Adet", "#4CAF50"),
                unsafe_allow_html=True,
            )
        with k3:
            st.markdown(
                kpi_card("💰 Toplam Maliyet:", f"${t_cost:,.0f}".replace(",", "."), "#FFC107"),
                unsafe_allow_html=True,
            )
        ...
        gb.configure_default_column(
            editable=False, resizable=True, sortable=True, filter=True, suppressMenu=False
        )

        # Tooltip özelliği
        gb.configure_column("Açıklama", tooltipField="Açıklama", width=320, flex=2)

        gb.configure_column("Ürün Kodu", width=140)
        gb.configure_column("Marka", cellStyle={"textAlign": "center"}, width=140)
        gb.configure_column("Ürün Grubu", cellStyle={"textAlign": "center"}, width=140)
        gb.configure_column("Güncel Stok", cellStyle={"textAlign": "center"}, width=110)
        if "Rezerve" in out_df.columns:
            gb.configure_column("Rezerve", cellStyle={"textAlign": "center"}, width=100)
        if "Satışa Açık" in out_df.columns:
            gb.configure_column("Satışa Açık", cellStyle={"textAlign": "center"}, width=110)
        gb.configure_column("Birim Maliyet", cellStyle={"textAlign": "right"}, width=130)
        gb.configure_column("Toplam Maliyet", cellStyle={"textAlign": "right"}, width=130)

        # Dinamik renklendirme (0 stoklu satırlar)
    ...
    st.error(f"Sistem Hatası: Lütfen 'requirements.txt' dosyasını kontrol edin. Detay: {e}")
